...
class Tool:
    def __init__(self, fn: Callable, name: Optional[str] = None, description: Optional[str] = None):
        ...
        for p_name, p_param in sig.parameters.items():
            annotation = p_param.annotation if p_param.annotation is not inspect.Parameter.empty else str
            default = ... if p_param.default is inspect.Parameter.empty else p_param.default
    ...
    def _build_definition(self) -> ToolDefinition:
        ...
        for f_name, f_field in self.InputModel.model_fields.items():
            prop = {"type": getattr(self.InputModel.model_fields[f_name].annotation, "__name__", "string")}
...
class ToolRegistry:
    ...
    def register(self, fn: Callable = None, *, name: Optional[str] = None, description: Optional[str] = None):
        def wrapper(f):
